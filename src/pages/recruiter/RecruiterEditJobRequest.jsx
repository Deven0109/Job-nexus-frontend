...
const RecruiterEditJobRequest = () => {
    ...
    const prefillLocationData = (reqData) => {
        const allCountries = Country.getAllCountries();
        // Case-insensitive match for country
        const countryObj = allCountries.find(c =>
            c.name.toLowerCase() === reqData.country?.toLowerCase() ||
            c.isoCode === reqData.country
        );
        ...
        if (countryObj) {
            ...
            const stateObj = states.find(s =>
                s.name.toLowerCase() === reqData.state?.toLowerCase() ||
                s.isoCode === reqData.state
            );
            ...
            if (stateObj) {
                ...
                setEditData({
                    ...reqData,
                    country: countryObj.name, // Normalize to standard name
                    countryCode: countryObj.isoCode,
                    state: stateObj.name, // Normalize to standard name
                    stateCode: stateObj.isoCode,
                    city: reqData.city // Keep as is, or could also normalize from cities list
                });
            } else {
                setCitiesList([]);
                setEditData({
            ...
            setStatesList([]);
            setCitiesList([]);
            setEditData({ ...reqData, countryCode: '', stateCode: '' });
        }
    };
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await getJobRequestById(id);
                if (res.success && res.data) {
                    const req = res.data.jobRequest;
                    setJobRequest(req);
                    setIsEditing(editMode);
                    if (editMode) {
                        prefillLocationData(req);
                    }
                }
            } catch (error) {
    ...
    const handleEdit = () => {
        prefillLocationData(jobRequest);
        setIsEditing(true);
    };
...
export default RecruiterEditJobRequest;
